...
const url = (u: string) => (/^(javascript|vbscript):/i.test(u) ? '#' : u);
...
const renderPara = (ls: string[]): string => {
  ...
  const dc = ls[1] && cells(ls[1]);
  if (dc && ls[0].includes('|') && dc.every((c) => /^:?-+:?$/.test(c))) {
    const al = dc.map((c) => (c[0] === ':' ? (c.at(-1) === ':' ? 'center' : 'left') : c.at(-1) === ':' ? 'right' : ''));
...
const renderFence = (ls: string[], info: string): string =>
  `<pre><code${info ? ` class="language-${info}"` : ''}>${ls[0] ? esc(ls.join('\n')) + '\n' : ''}</code></pre>\n`;

// items: [indent, ordered, text, startNumber]
const renderList = (items: any[][]): string => {
  const f = items[0],
    tag = f[1] ? 'ol' : 'ul';
  let h = `<${tag}${f[3] > 1 ? ` start="${f[3]}"` : ''}>\n`,
    i = 0;
  while (i < items.length) {
    ...
    h += `<li>${il(it[2]).replace(/^\[([ xX])\]( |$)/, (_, c) => `<input type="checkbox" disabled${c === ' ' ? '' : ' checked'}> `)}${
      kids[0] ? '\n' + renderList(kids).slice(0, -1) : ''
    }</li>\n`;
    i = j;
  }
  return h + `</${tag}>\n`;
...
export const createParser = (): Parser => {
  ...
  const draft = (): string => R[open]?.(lines, info) ?? '';
  ...
  const feed = (line: string): string => {
    ...
    if ((m = line.match(/^\s{0,3}(#{1,6})(?:\s+|$)(.*)$/)))
      return close() + `<h${m[1].length}>${il(m[2].replace(/(\s+#+)?\s*$/, ''))}</h${m[1].length}>\n`;
    ...
    if ((m = line.match(/^(\s*)(?:(\d+)[.)]|[-*+])\s+(.*)$/))) {
      if (open !== 'l') h = swap('l');
      lines.push([m[1].length, +!!m[2], m[3], +m[2] || 0]);
      return h;
    }
    if (open !== 'p') h = swap('p');
